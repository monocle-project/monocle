...
Value getnewstealthaddress(const Array& params, bool fHelp)
{
    ...
    if (!pwalletMain->IsLocked()){
        ...
        // Check scan secret and spend secret in wallet, if they don't have generate and put
        // them back to wallet
        if(!(pwalletMain->GetScanSecret(scanSecret) && pwalletMain->GetSpendSecret(spendSecret))){
            pwalletMain->SetScanSecret(scanSecret);
            pwalletMain->SetSpendSecret(spendSecret);
        }
