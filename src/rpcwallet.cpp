...
Value getinfo(const Array& params, bool fHelp)
{
    ...
    obj.push_back(Pair("moneysupply", ValueFromAmount(pindexBest ? pindexBest->nMoneySupply : 0)));
...
Value getnewstealthaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
                ...
                "Returns a new Monocle Stealth address for receiving payments.  "
                "If [account] is specified (recommended), it is added to the address book "
                "so payments received with the address will be credited to [account].");
...
Value liststealthaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
              ...
              "Returns a list of Monocle Stealth address"
              "If [account] is specified (recommended), it will return only for that account"
              "NOTE: This account doesn't relate to Monocle address accounts");
...
Value sendtostealthaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            "sendtostealthaddress <monoclestealthaddress> <amount> [comment] [comment-to]\n"
            "<amount> is a real and is rounded to the nearest 0.00000001"
            + HelpRequiringPassphrase());
...
Value sendstealthfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
            "sendstealthfrom <fromaccount> <tomonoclestealthaddress> <amount> [minconf=1] [comment] [comment-to]\n"
            "<amount> is a real and is rounded to the nearest 0.00000001"
            + HelpRequiringPassphrase());
